//! VGG-Tiny CNN Training for MNIST
//!
//! This binary trains a VGG-style (Simonyan & Zisserman, 2014) CNN on MNIST.
//! VGG-Tiny: simplified version using only 3x3 convolutions.
//!
//! Architecture:
//! - Input: 28x28x1 (grayscale)
//! - Block 1: Conv 1→32, 3x3 (×2) + MaxPool → 14x14x32
//! - Block 2: Conv 32→64, 3x3 (×2) + MaxPool → 7x7x64
//! - Flatten → 3136
//! - FC: 3136 → 128 → 10
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  VGG-Tiny CNN for MNIST (Simonyan & Zisserman, 2014 style)   ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");

    let model_path = "src/vgg_model.bin";
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // 2. BUILD VGG-Tiny CNN
    // ═══════════════════════════════════════════════════════════════════════
    println!("🔧 Building VGG-Tiny CNN...\n");

    // VGG-style: stacked 3x3 convolutions
    let cnn = Sequential::named("VGG-Tiny")
        // Block 1: 2x Conv 3x3, 32 filters
        .add_conv2d(Conv2D::new(1, 32, 3, 1, 1)) // 28x28x32
        .add_activation(ActivationLayer::relu())
        .add_conv2d(Conv2D::new(32, 32, 3, 1, 1)) // 28x28x32
        .add_activation(ActivationLayer::relu())
        .add_maxpool(MaxPool2D::new(2, 2)) // 14x14x32
        // Block 2: 2x Conv 3x3, 64 filters
        .add_conv2d(Conv2D::new(32, 64, 3, 1, 1)) // 14x14x64
        .add_activation(ActivationLayer::relu())
        .add_conv2d(Conv2D::new(64, 64, 3, 1, 1)) // 14x14x64
        .add_activation(ActivationLayer::relu())
        .add_maxpool(MaxPool2D::new(2, 2)) // 7x7x64
        // Flatten
        .add_flatten();
    ...
    match save_model_binary(classifier, acc, total, Some(norm_stats), model_path) {
        Ok(_) => {
            ...
            println!("\n╔══════════════════════════════════════════════════════════════╗");
            println!("║             VGG-Tiny Training Complete! 🎉                   ║");
            println!("╚══════════════════════════════════════════════════════════════╝");
        }
        Err(e) => {
